#!/usr/bin/env python3
"""
scientific_expert_bot_v4_ollama_hybrid.py
====================================================
MULTI-SPECIALIST PROFESSIONAL SCIENTIFIC AI BOT v4.0 HYBRID
Fully working numerical computing with real calculations
Physics + Chemistry + Mathematics Specialists
+ PROFESSIONAL NATURAL LANGUAGE CHAT MODE (Rule-based + Ollama LLM fallback)
Powered by NumPy + SciPy + SymPy + Ollama Cloud
All code, comments, and output in English + Turkish support in LLM mode
Easy to run on tablet (Pydroid 3, VS Code, online Python, etc.)
No compilation needed
====================================================
Professional Edition v4.0 HYBRID
- All v3 calculation power
- Natural language scientific chat (rule-based for precision)
- Real LLM fallback via your Ollama Cloud API Key (for open scientific conversation)
- Best of both worlds: Precise calculations + Intelligent AI chat
====================================================
"""
...
# Ollama Cloud için (gerçek LLM konuşma için)
try:
...
def main_menu():
    ...
    while True:
        print("\n" + "="*45)
        print("MAIN MENU v4.0 HYBRID")
        print("1. Physics Specialists")
        print("2. Chemistry Specialists")
        print("3. Mathematics & Numerical Tools")
        print("4. Professional Chat Mode (Natural Language - Rule-based)")
        print("5. Ollama Cloud Scientific Chat (Real LLM with your API Key)")
        print("6. About this Professional Scientific AI")
        print("0. Exit")
        ...
        if choice == "1":
            ...
            ollama_scientific_chat()
        elif choice == "6":
            about_bot()
        elif choice == "0":
            print("\nThank you for using the Professional Scientific AI Bot v4.0 HYBRID!")
...
def ollama_scientific_chat():
    """
    Ollama Cloud ile gerçek LLM destekli bilimsel sohbet.
    Kullanıcının API Key'i ile ollama.com üzerinden güçlü model kullanılır.
    Konuşma geçmişi tutulur, streaming cevap verilir.
    v4 Hybrid'te scientific bot ile entegre çalışır.
    """
    print("\n" + "="*70)
    print(">>> OLLAMA CLOUD - GERÇEK LLM BİLİMSEL SOHBET (v4 HYBRID) <<<")
    print("Senin API Key'in ile ollama.com'daki güçlü modellerle konuş.")
    print("Bilimsel sorular, açıklamalar, tartışmalar için ideal.")
    print("")

    if not OLLAMA_AVAILABLE:
        print("❌ 'ollama' kütüphanesi yüklü değil.")
    ...
    if not api_key:
        print("⚠️ OLLAMA_API_KEY environment variable bulunamadı.")
        print("   Güvenli yöntem: export OLLAMA_API_KEY=senin_keyin")
        api_key = input("API Key'ini yapıştır (bu oturum için): ").strip()
        if not api_key:
            print("Key olmadan çalışamaz.")
            return
    ...
    try:
        ...
        print("✅ Ollama Cloud bağlantısı başarılı!")
    except Exception as e:
        print(f"❌ Bağlantı hatası: {e}")
        return

    # Model seç
    print("\nPopüler modeller: gpt-oss:120b, llama3.2, gemma2, qwen2.5 vb.")
    model = input("Model adı (varsayılan gpt-oss:120b): ").strip() or "gpt-oss:120b"

    print(f"\n🚀 Model: {model}")
    print("Sohbete başla! ('çık', 'exit', 'menu' yazarak çık)")
    print("-" * 70)

    messages = [
        {
            "role": "system",
            "content": "Sen uzman bir bilim insanı ve öğretmen gibi konuşan, samimi, net ve bilgili bir Türkçe/İngilizce yapay zeka asistanısın. "
                       "Kullanıcının bilimsel sorularına doğru, adım adım ve eğitici cevaplar ver. "
                       "Gerektiğinde örnek ver, formül yaz, basit tut. "
                       "Bilmediğin şeyde 'Bilmiyorum, ama şöyle düşünebiliriz' de."
        }
    ]
    ...
    while True:
        try:
            ...
            if not user_input:
                continue
            if user_input.lower() in ["çık", "exit", "quit", "menu", "back"]:
                print("Ana menüye dönülüyor...")
                break

            messages.append({"role": "user", "content": user_input})

            print("\n🤖 LLM Asistan: ", end="", flush=True)
            full_response = ""
            ...
            # Bellek yönetimi (çok uzun olursa kısalt)
            if len(messages) > 18:
    ...
    print("\nOllama sohbeti bitti. Ana menüye dönülüyor.")


def about_bot():
    print("\n" + "="*60)
    print(">>> ABOUT THIS PROFESSIONAL SCIENTIFIC AI BOT v4.0 HYBRID <<<")
    print("="*60)
